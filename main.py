...
class Utils:
    @staticmethod
    def sprint(content: str, status: str) -> None:
        colour = Fore.CYAN
        if status == "y":
...
class DeviceToken:
    ...
    def get_proofkey(priv_key: ec.EllipticCurvePrivateKey):
        def int_to_base64(x: int, length=32, byteorder="big") -> str:
            return (
                base64.urlsafe_b64encode(x.to_bytes(length, byteorder))
                .decode("ascii")
                .replace("=", "")
            )
...
def pull():
    ...
    try:
        proxies = {
            "https": "http://" + random.choice(open("proxies.txt").read().splitlines())
        }
    except:
        proxies = None
    s.proxies = proxies
    ...
    while True:
        try:
            ...
            Utils.sprint(e, "r")
            return
    if "https://account.live.com/proofs/Add" in response.url:
        headers = {
        ...
        data = {
            ...
            "DisplayPhoneNumber": "",
            "EmailAddress": "",
            "canary": response.text.split('id="canary" name="canary" value="')[1].split(
                '"'
            )[0],
            "action": "Skip",
            "PhoneNumber": "",
    ...
    try:
        ...
        Utils.sprint("[-] Invalid Account", "r")
        Utils.remove_content("accounts.txt", ms_creds)
        return
    ...
    if "resource" in response.json():
        ...
        Utils.sprint(
            f"[-] Failed to Fetch -=> {response.text} | code: {response.status_code}",
            "y",
        )
        if "ineligible" in response.text:
            Utils.remove_content("accounts.txt", ms_creds)
        return
    comde = str(link).split("/")[3]
    Utils.sprint(f"[+] Successfully Fetched Link -=> {comde[:15]}", "c")
    Utils.remove_content("accounts.txt", ms_creds)
